...
metasel=meta[meta$ADdiag3types!='lateAD',]
...
for (c in ctps){
  ...
  summaryCond <- summary(zlmCond, doLRT = "condearlyAD")
  
  summaryDt <- summaryCond$datatable
  res <- merge(summaryDt[contrast=='condearlyAD' & component=='H',.(primerid, `Pr(>Chisq)`)], #hurdle P values
               summaryDt[contrast=='condearlyAD' & component=='logFC', .(primerid, coef, ci.hi, ci.lo)], by='primerid') #logFC coefficients
  
  res[,fdr:=p.adjust(`Pr(>Chisq)`, 'fdr')]
  res=as.data.frame(res)
  
  write.table(res,file=paste(paste(fid,c,sep = '.'),'.non_early.MASTres.txt',sep=''),sep = '\t',row.names = F,quote = F)
}
